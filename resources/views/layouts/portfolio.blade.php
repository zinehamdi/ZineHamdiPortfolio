...
<head>
    ...
    <meta name="keywords"
        content="Laravel Developer, Full Stack Developer, AI Integration, Web Development Tunisia, PHP Developer, Vue.js, Tailwind CSS, Olive Oil Branding, Scrum Master">
    ...
    <link
        href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800;900&family=JetBrains+Mono:wght@400;500;600&family=IBM+Plex+Sans+Arabic:wght@400;600;700;800&display=swap"
        rel="stylesheet">
    ...
    <script
        type="application/ld+json">{!! json_encode($website, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) !!}</script>
    <script
        type="application/ld+json">{!! json_encode($person, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) !!}</script>
    <script
        type="application/ld+json">{!! json_encode($services, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) !!}</script>
</head>
...
<body class="bg-[#0a0a0f] text-[#E8E8E8] min-h-screen overflow-x-hidden">
    ...
    <div
        class="fixed inset-0 bg-[radial-gradient(circle_at_50%_0%,_rgba(0,255,136,0.05),_transparent_70%)] z-0 pointer-events-none">
    </div>

    <!-- Skip Link -->
    <a href="#main"
        class="sr-only focus:not-sr-only focus:fixed focus:top-3 focus:left-3 focus:z-[100] bg-[#00FF88] text-[#0a0a0f] px-4 py-2 rounded-lg font-bold">Skip
        to content</a>
    ...
    <!-- Modern Sidebar -->
    <aside id="sidebar"
        class="fixed left-0 top-0 bottom-0 w-72 sm:w-80 mlp-card-strong mlp-bg-metal/80 mlp-glass-subtle border border-mlp-border-subtle/70 z-50 transform -translate-x-full transition-transform duration-500 ease-out lg:translate-x-0"
        ...
        <div class="p-6 h-full flex flex-col relative">
            ...
            <div class="absolute bottom-8 left-4 w-24 h-24 bg-mlp-laser-purple/10 opacity-50 blur-3xl rounded-full">
            </div>

            <!-- Profile Section -->
            <div class="text-center mb-8 relative z-10">
                <div
                    class="w-28 h-28 mx-auto rounded-full overflow-hidden border border-mlp-border-subtle/70 shadow-mlp-laser-green mlp-laser-orbit">
                    <img src="{{ $profileImg }}" alt="Profile" class="w-full h-full object-cover object-top"
                        style="object-position: center 15%;">
                </div>
                <h2 class="mt-4 text-xl font-bold text-mlp-text-main mlp-metal-sheen">
                    {{ __('common.brand') ?: 'ZINDEV' }}</h2>
            ...
            <nav class="flex-1 space-y-2" aria-label="Section navigation">
                ...
                @foreach($navItems as $item)
                            @php 
                                // Blog is a separate page, not a section on homepage
                                if ($item['key'] === 'blog') {
                                    $href = route('blog', ['locale' => app()->getLocale()]);
                                } else {
                                    $href = $isHome ? '#' . $item['key'] : $base . '#' . $item['key'];
                                }
                            @endphp
                            <a href="{{ $href }}" 
...
c           om/in/zinehamdi" target="_blank" rel="noopener noreferrer" aria-label="LinkedIn profile" class="text-[#A0A0A0] hover:text-[#00FF88] transition-colors">

                               <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M19 0h-14c-2.761
                0-5 2.239-5 5v14c
                0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2 .2
            38-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/></svg>
                </a>
    ...
    <!-- Main Content -->
    <main id="main" class="pt-16 lg:ml-80 lg:pl-6 min-h-screen relative z-10">
        @if(session('status'))
