...
<body class="bg-[#dbdbd7] text-[#1b1b18] min-h-screen overflow-x-hidden">
    ...
    <aside id="sidebar" class="fixed left-0 top-0 bottom-0 lg:top-16 w-72 sm:w-80 bg-[#FFA400] z-50 shadow-2xl transform -translate-x-full transition-transform duration-300 ease-in-out lg:translate-x-0" aria-label="Main navigation">
    ...
    <!-- Mobile Overlay -->
    <div id="sidebar-overlay" class="fixed inset-0 lg:top-16 bg-black/40 backdrop-blur-sm z-40 hidden lg:hidden" aria-hidden="true"></div>
    ...
    @php
        ...
        $profileImgHeader = file_exists(public_path('images/home.jpg'))
            ...
            : (file_exists(public_path('home.jpg'))
                ...
                : (file_exists(public_path('images/profili1.jpg')) ? asset('images/profili1.jpg') : asset('favicon.ico')));
    @endphp
    <header class="fixed left-0 right-0 top-0 z-30 bg-[#1b1b18] text-white/90 backdrop-blur supports-[backdrop-filter]:bg-[#1b1b18]/90 h-16 flex items-center px-3 sm:px-6 lg:pl-80 gap-4 shadow-lg">
        <!-- Mobile hamburger -->
    ...
    <div class="ml-auto flex items-center gap-2 sm:gap-3">
            ...
            <x-lang-switcher class="flex gap-1 md:gap-2 md:flex" tone="dark" />
            <!-- Chat button removed -->
        </div>
    </header>
    <main id="main" class="pt-16 lg:ml-80 min-h-screen">
        @if(session('status'))
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function scrollToWithOffset(el) {
                ...
                const y = el.getBoundingClientRect().top + window.scrollY - headerH - 8; // small breathing space
                window.scrollTo({ top: Math.max(y, 0), behavior: 'smooth' });
            }

            // Intercept sidebar clicks for same-page anchors
            ...
            // Adjust scroll for hash on load and when hash changes (e.g., deep links, cross-page nav)
            function adjustHashScroll() {
                if (location.hash && location.hash.length > 1) {
                    const id = decodeURIComponent(location.hash.substring(1));
                    const el = document.getElementById(id);
                    if (el) {
                        // Use rAF to wait for layout, then scroll
                        requestAnimationFrame(() => scrollToWithOffset(el));
                    }
                }
            }
            window.addEventListener('hashchange', adjustHashScroll);
            // After initial layout, fix scroll position if a hash is present
            ...
            function updateActiveNav() {
                const header = document.querySelector('header');
                const headerH = header ? header.offsetHeight : 0;
                const scrollPos = window.scrollY + headerH + 16;
    ...
    <style>
        @media (min-width: 1024px) {
            /* Sidebar height matches content area (viewport - fixed header) */
            #sidebar { height: calc(100vh - 4rem); }
        }
        /* Account for fixed header when anchoring to sections */
        section[id] { scroll-margin-top: 7rem; }
        @media (min-width: 1024px) {
            /* Header is still present; keep margin */
            section[id] { scroll-margin-top: 7rem; }
        }
