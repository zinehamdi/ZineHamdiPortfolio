<section id="blog" class="py-12 relative mlp-bg-root border-t border-mlp-border-subtle/50">
    @php
        ...
        $dbPosts = \App\Models\Post::published()->locale()->latest('published_at')->take(3)->get();

        // Fallback comic backgrounds
        $blogComics = [
        ...
        // If no database posts, use lang file articles
        if ($dbPosts->isEmpty()) {
            $articles = __('blog.articles');
            $useLangArticles = true;
        } else {
            $useLangArticles = false;
        }
    @endphp
    <div class="container mx-auto px-4 sm:px-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8" data-animate="blog-grid">
            @if($useLangArticles)
                {{-- Use lang file articles as fallback --}}
                @foreach($articles as $index => $article)
                    <article
                        ...
                        <div class="h-48 relative overflow-hidden">
                            <img src="{{ asset('images/' . $blogComics[$index]) }}" alt="{{ $article['category'] }}"
                        ...
                        <div class="h-48 relative overflow-hidden">
                            @if($post->featured_image)
                                <img src="{{ asset($post->featured_image) }}" alt="{{ $post->title }}"
                                    class="absolute inset-0 w-full h-full object-cover opacity-50 group-hover:opacity-70 group-hover:scale-110 transition-all duration-700">
                            @else
                                <img src="{{ asset('images/' . ($blogComics[$index] ?? $blogComics[0])) }}"
                                    alt="{{ $post->category }}"
                                    class="absolute inset-0 w-full h-full object-cover opacity-40 group-hover:opacity-50 group-hover:scale-110 transition-all duration-700">
                            @endif
                            <div class="absolute inset-0 bg-gradient-to-br from-black/60 via-black/40 to-transparent"></div>
