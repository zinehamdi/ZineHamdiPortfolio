...
@php
  $bgImage = 'images/bgPortfolioImage.png';
...
@endphp
<section id="home" class="relative min-h-screen flex items-center pt-20 lg:pt-0 bg-center bg-cover" style="background-image: url('{{ asset($bgImage) }}');" @if($rtl) dir="rtl" @endif>
  <div class="relative max-w-7xl mx-auto px-4 sm:px-6 w-full">
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8 items-start lg:items-stretch">
      ...
      <div id="introCard" class="bg-white p-6 sm:p-8 lg:p-12 rounded-2xl section-shadow transition-all duration-300 h-full">
  ...
  <div id="vlog" class="bg-white p-6 sm:p-8 lg:p-12 rounded-2xl section-shadow transition-all duration-300 order-first lg:order-last h-full"
  x-data="vlogCard">
    @php
  ...
  $imgPath    = trim((string)($promo['image'] ?? 'images/hero-promo.jpg'));
  ...
  <div class="w-full" x-ref="card">
      ...
      <div x-ref="textWrap"
        class="text-[#1b1b18]/80 leading-relaxed overflow-hidden transition-all duration-300"
        :style="styleTextWrap()">
              {{ $promoText }}
            </div>
            <!-- Fade overlay when collapsed -->
      <div class="pointer-events-none absolute left-0 right-0 bottom-0 h-8"
        x-show="!expanded"
                 style="background: linear-gradient(to bottom, rgba(255,255,255,0), rgba(255,255,255,1));"></div>

            <button type="button"
                    class="mt-2 text-sm font-semibold underline underline-offset-2 text-[#1b1b18]/70 hover:text-[#1b1b18]"
        @click="toggle()">
        <span x-show="!expanded" aria-hidden="true">{{ $tr('common.actions.show_more', 'Show more') }}</span>
        <span x-show="expanded" aria-hidden="true">{{ $tr('common.actions.show_less', 'Show less') }}</span>
            </button>
          </div>
          <div class="mt-4">
